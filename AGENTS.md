# Project context for AI coding agents — nerocompanion

> This and `CLAUDE.md` are kept identical; update both together.

## The mod

- **NeroCompanion** — part of the Neroland sci-fi Minecraft mod ecosystem, built on **Neroland Core**.
...
- Mod id: **`nerocompanion`** (matches the registry namespace + every loader manifest). Package root:
  `za.co.neroland.nerocompanion`. Author: **Neroland**.
...
- Loader entry points: `NeroCompanionFabric` (+ `NeroCompanionFabricClient`), `NeroCompanionForge`,
  `NeroCompanionNeoForge` — each calls `NeroCompanionCommon.init()` during construction.
- NeoForge/Forge debug tasks use `-PnerocompanionDebug`; Fabric Loom honours Gradle `--debug-jvm`.

## IDE (VS Code) run & debug

- Workspace: **`nerocompanion.code-workspace`** (single-root `"."`). Import the Stonecutter nodes as **static
  ...
  VS Code. Per-node Eclipse project names are `nerocompanion-<loader>-<mc>`.
...
- This mod has its own **dedicated wiki** in `wiki/` at the repo root: the player- and
  contributor-facing docs for NeroCompanion (features, blocks/items, machines, progression, recipes, FAQ).
...
- The wiki is **per-mod** — document only NeroCompanion here; cross-mod / ecosystem concepts belong in the
